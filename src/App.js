...
const TaskList = () => {
    ...
    const addTask = () => {
        if (newTask.trim()) {
            axios.post('http://localhost:5000/api/tasks', { title: newTask, completed: false }).then((response) => {
    ...
    const saveEdit = () => {
        if (editedTitle.trim()) {
            axios.put(`http://localhost:5000/api/tasks/${editingTask}`, { title: editedTitle }).then((response) => {
    ...
    return (
        <div className="flex flex-col items-center bg-gray-100 min-h-screen py-8 px-4 sm:px-6 md:px-8 lg:px-16">
            <h1 className="text-5xl font-bold text-gray-800 mb-6 text-center">To Do List</h1>
